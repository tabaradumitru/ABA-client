...
export class AllRequestsComponent implements OnInit {
  ...
  dataResponse: PaginatedResponse<Request[]> = {} as PaginatedResponse<Request[]>;
  ...
  ngOnInit(): void {
    ...
    forkJoin([$statuses, $requests, $activities, $localities]).subscribe(response => {
      ...
      this.requests = response[1].content;
      this.dataResponse.pageSize = response[1].pageSize;
      this.dataResponse.totalCount = response[1].totalCount;
      this.first = (response[1].currentPage - 1) * response[1].pageSize;
  ...
  onRowSelect(selectedRequest: Request): void {
    // TODO: open dialog with request details
